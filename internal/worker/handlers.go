...
func (h *ScrapeTaskHandler) ProcessTask(ctx context.Context, t *asynq.Task) error {
	...
	h.logger.Info("Processing scrape task", "url", payload.URL, "render", payload.Render, "task_id", t.ResultWriter().TaskID())

	result, err := h.scraper.Scrape(ctx, payload.URL, payload.Render)
	if err != nil {
