...
- **Bun Worker Notes**:
  - Bun’s `Worker` API is **experimental**, especially around termination semantics.
  - `worker.unref()` can allow the process to exit if the worker is idle.
  - Consider `smol: true` for lower memory usage if worker memory pressure is high.
